...
def index():
    ...
    sql = """SELECT g.*, p.display_name AS author_name
             FROM gear_items g LEFT JOIN participants p ON p.id = g.author_id
             WHERE 1=1"""
    args = []
    ...
    return render_template(
        "gear/index.html",
        items=items, categories=GEAR_CATEGORIES, status_labels=STATUS_LABELS,
        type_filter=type_filter, cat=cat, q=q,
    )
    ...
    if request.method == "POST":
        ...
        db.execute(
            """UPDATE gear_items SET type=?, item_name=?, category=?, quantity=?, description=?,
                pickup_return_notes=?, contact_preference=?, status=?, updated_at=CURRENT_TIMESTAMP
               WHERE id=?""",
            (data["type"], data["item_name"], data["category"], data["quantity"],
             data["description"], data["pickup_return_notes"], data["contact_preference"],
             status, item_id),
