...
def index():
    ...
    sql = """SELECT m.*, p.display_name AS author_name
             FROM meal_ideas m LEFT JOIN participants p ON p.id = m.author_id WHERE 1=1"""
    args = []
    if meal_type:
        sql += " AND m.meal_type = ?"; args.append(meal_type)
    if diet:
        sql += " AND m.dietary_tags LIKE ?"; args.append(f"%{diet}%")
    if cleanup:
        sql += " AND m.cleanup_level = ?"; args.append(cleanup)
    if q:
    ...
    return render_template("meals/index.html", items=items, meal_types=MEAL_TYPES,
                           dietary_tags=DIETARY_TAGS, cleanup_levels=CLEANUP_LEVELS,
    ...
    if request.method == "POST":
        ...
        db.execute(
            """INSERT INTO meal_ideas (title, meal_type, serves, description, prep_ahead_notes,
                cooking_instructions, equipment_needed, dietary_tags, cleanup_level, author_id, image_path)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (data["title"], data["meal_type"], data["serves"], data["description"],
             ...
             ",".join(data["dietary"]), data["cleanup_level"], g.user["id"], image_path),
        )
    ...
    if request.method == "POST":
        ...
        db.execute(
            """UPDATE meal_ideas SET title=?, meal_type=?, serves=?, description=?,
               ...
               dietary_tags=?, cleanup_level=?, image_path=?, updated_at=CURRENT_TIMESTAMP WHERE id=?""",
            (data["title"], data["meal_type"], data["serves"], data["description"],
             data["prep_ahead_notes"], data["cooking_instructions"], data["equipment_needed"],
             ",".join(data["dietary"]), data["cleanup_level"], image_path, item_id),
        )
